 ...
 * @details Testing PWM signals with dead time.
 *
 * @verbatim
...
int main(void) {
	...
	// Initialize PWM with arbitrary dead time value
	PWM_Config(127);

	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);

	// Configure USER button on F4 discovery board as input
...
/**
 * @brief Callback function for soft timer.
 */
void softTimerCallback(void) {
	...
	static uint8_t deadTime; // dead time value

	if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0)) { // read USER button

		if (deadTime <= 230) {
			deadTime += 5; // Increase dead time and decrease duty cycle of signals
		} else {
			deadTime += 1; // smaller step for longer dead time (very small duty cycle)
		}
